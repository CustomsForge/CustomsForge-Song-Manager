...
namespace CFSMTests
{
...
#if NCALCTest
    [TestClass]
    public class NCalcExpressionTest
    {
        /// <summary>
        ...
        public void ShouldEvaluateConditionnal()
        {
            ...
            eif = new Expression("true == false");
            Assert.AreEqual(false, eif.Evaluate());
    ...
    {
        [TestMethod]
        //TestTOC_RSTK will always fail
        public void TestTOC_RSTK()
        {
            Random r = new Random();
            
            using (var l = new CFSM.Utils.NoCloseStreamList())
            {
                CFSM.Utils.NoCloseStream pwrite = l.NewStream();
                using (var p = new RocksmithToolkitLib.PSARC.PSARC())
                {
                    for (int i = 0; i < 10; i++)
                    {
                        byte[] b = new byte[50];
        ...
        public void TestTOC_CFSMUTILS()
        {
            ...
            using (var l = new CFSM.Utils.NoCloseStreamList())
            {
                ...
                using (var p = new CFSM.Utils.PSARC.PSARC())
                {
                    for (int i = 0; i < 10; i++)
                    {
                        byte[] b = new byte[50];
