...
namespace DataGridViewTools
{
    ...
    {
        public OperatorExpression(string name, string value) : base(name, value)
        {
        }

        public override string Compile(string propName)
        {
            return String.Format("([{1}] {0} {2})", FunctionName, propName, ((CueTextBox) FEditor).Text);
        }
    }

    public class NoExpression : Expression
    {
        public NoExpression(string name, string value) : base("None", "")
        {
        }
