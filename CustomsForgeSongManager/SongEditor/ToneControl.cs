...
namespace CustomsForgeSongManager.SongEditor
{
    ...
    public partial class ToneControl : UserControl
    {
        ...
        private void InitializeToneInformation()
        {
            ...
            descriptorCombo.SelectedValueChanged += (sender, e) => UpdateToneDescription((ComboBox) sender);
        }
        ...
        private void InitializeComboBoxes()
        {
            ...
            var amps = allPedals.Where(p => p.TypeEnum == PedalType.Amp).OrderBy(p => p.DisplayName).ToArray();
            var cabinets = allPedals.Where(p => p.TypeEnum == PedalType.Cabinet).OrderBy(p => p.DisplayName).ToArray();
            var loopRackPedals = allPedals.Where(p => p.TypeEnum == PedalType.Rack).OrderBy(p => p.DisplayName).ToArray();
            var prePedals = allPedals.Where(p => p.TypeEnum == PedalType.Pedal).OrderBy(p => p.DisplayName).ToArray();
            var postPedals = allPedals.Where(p => p.TypeEnum == PedalType.Pedal).OrderBy(p => p.DisplayName).ToArray();
