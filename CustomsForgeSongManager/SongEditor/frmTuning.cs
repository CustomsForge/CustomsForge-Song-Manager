...
namespace CustomsForgeSongManager.SongEditor
{
    public partial class frmTuning : Form
    {
        ...
        private void okButton_Click(object sender, EventArgs e)
        {
            if (Tuning == null)
            {
                MessageBox.Show("All fields are required!", Constants.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            if (IsBass && Tuning.Tuning.String4 == 0 && Tuning.Tuning.String5 == 0)
                if (MessageBox.Show(String.Format("Are strings 4 and 5 really 0?{0}We recommend filling in all strings fields (to cover guitar tuning).{0}To avoid two of the same tuning for different instruments.{0}{0}Would you like to cancel save and correct it now?{0}", Environment.NewLine), Constants.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    return;
