...
namespace CustomsForgeSongManager.SongEditor
{
    public partial class frmArrangement : Form
    {
        ...
        private ucArrangements _parentControl = null;
        ...
        public void LoadArrangementData(string xmlFilePath)
        {
            ...
            {
                var pcSongInfo = _parentControl.SongData.SongInfo;

                if (String.IsNullOrEmpty(pcSongInfo.SongDisplayName)) pcSongInfo.SongDisplayName = _song2014.Title ?? String.Empty;
                if (String.IsNullOrEmpty(pcSongInfo.SongDisplayNameSort)) pcSongInfo.SongDisplayNameSort = _song2014.SongNameSort.GetValidSortableName() ?? pcSongInfo.SongDisplayName.GetValidSortableName();
                if (pcSongInfo.AverageTempo == 0) pcSongInfo.AverageTempo = (Int32)_song2014.AverageTempo;
                if (String.IsNullOrEmpty(pcSongInfo.Artist)) pcSongInfo.Artist = _song2014.ArtistName ?? String.Empty;
                if (String.IsNullOrEmpty(pcSongInfo.ArtistSort)) pcSongInfo.ArtistSort = _song2014.ArtistNameSort.GetValidSortableName() ?? pcSongInfo.Artist.GetValidSortableName();
                if (String.IsNullOrEmpty(pcSongInfo.Album)) pcSongInfo.Album = _song2014.AlbumName ?? String.Empty;
                if (pcSongInfo.SongYear == 0) pcSongInfo.SongYear = Convert.ToInt32(_song2014.AlbumYear);
                if (String.IsNullOrEmpty(_parentControl.SongData.Name)) _parentControl.SongData.Name = String.Format("{0}{1}", pcSongInfo.Artist.GetValidAcronym(), pcSongInfo.SongDisplayName).GetValidKey(pcSongInfo.SongDisplayName);
                ...
                if (String.IsNullOrEmpty(pcSongInfo.AlbumSort))
                {
                    ...
                    if (useDefaultAuthor)
                        ...
                        pcSongInfo.AlbumSort = _song2014.AlbumNameSort.GetValidSortableName() ?? pcSongInfo.Album.GetValidSortableName();
                }
            }
        ...
        public bool AddXmlArrangement(string xmlFilePath)
        {
            ...
            else // add Instrument Arrangement
            {
                ...
                catch (Exception ex)
                {
                    ...
                    RocksmithToolkitLib.Extensions.BetterDialog2.ShowDialog(diaMsg, "AddXmlArrangement", null, null, "OK", Bitmap.FromHicon(SystemIcons.Warning.Handle), "Warning", 150, 150);
                    return false;
                }
            }
            return true;
        ...
        {
            var pcArrangements = _parentControl.SongData.Arrangements;
        ...
        private void SetupTones(Arrangement arr)
        {
            ...
            var pcTones = _parentControl.SongData.TonesRS2014;
        ...
        private void ShowTuningForm(ArrangementType selectedType, TuningDefinition tuning)
        {
            if (tuning == null)
            {
                MessageBox.Show("Pick a tuning definition to start editing.\r\n (DEBUG: Current tuning is Null)", "ShowTuningForm", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            using (var ofd = new OpenFileDialog())
            {
                ...
                if (IsAlreadyAdded(XmlPath))
                {
                    MessageBox.Show(@"XML Arrangement: " + Path.GetFileName(XmlPath) + @"   " + Environment.NewLine +
                                    ...
                                    "btnBrowseXml", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Xml Arrangement file path is not valid.", "btnOk_Click", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtXmlPath.Focus();
                return;
            }
            ...
            if (_gameVersion != GameVersion.RS2012)
            {
                ...
                {
                    MessageBox.Show("You did not select a Gameplay Path for this arrangement.", "btnOk_Click", MessageBoxButtons.OK, MessageBoxIcon.Warning);
