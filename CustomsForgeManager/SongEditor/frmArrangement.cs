...
namespace CustomsForgeManager.SongEditor
{
    ...
    public partial class frmArrangement : Form
    {
        ...
        private void ShowTuningForm(ArrangementType selectedType, TuningDefinition tuning)
        {
            if (tuning == null)
            {
                MessageBox.Show("Pick a tuning definition to start editing.\r\n (Current tuning is Null)", CustomsForgeManagerLib.Objects.Constants.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void OkButton_Click(object sender, EventArgs e)
        {
            ...
            if (!File.Exists(xmlfilepath))
                if (MessageBox.Show("Xml Arrangement file path is not valid.", CustomsForgeManagerLib.Objects.Constants.ApplicationName, MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    XmlFilePath.Focus();
                    return;
                }
        ...
        public bool LoadArrangementData(string xmlfilepath)
        {
            ...
            if (!ReferenceEquals(xmlSong, null))
            {
                ...
                if (String.IsNullOrEmpty(parentControl.SongData.Name)) parentControl.SongData.Name = pcSongInfo.Artist.Acronym() + pcSongInfo.SongDisplayNameSort;
            }
        ...
        private void ApplyBassFix(Arrangement arr)
        {
            if (arr.TuningPitch.Equals(220.0))
            {
                MessageBox.Show("This song is already at 220Hz pitch (bass fixed applied already?)", CustomsForgeManagerLib.Objects.Constants.ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
